...
// Simulate the test function with multithreading
pub fn test() {
    use SortAlgorithm::*;
    ...
    let sorting_jobs = (0..300)
        .map(|_| BubbleSort(data.clone()))
        .chain((0..300).map(|_| MergeSort(data.clone())))
        .chain((0..300).map(|_| QuickSort(data.clone())))
        .collect::<Vec<_>>();

    // Iterate over each sorting job and use match to select the algorithm
    for mut job in sorting_jobs {
        match job {
            BubbleSort(ref mut arr) => {
                *arr = bubble_sort(arr.clone());
            }
            MergeSort(ref mut arr) => {
                *arr = merge_sort_upper(arr.clone());
            }
            QuickSort(ref mut arr) => {
                *arr = quick_sort(arr.clone());
            }
        }
    }
}
